...
def combine_selected_columns(csv_files):
    ...
    for csv_file in csv_files:
        ...
        # Extract the "LOW" column if it exists, otherwise create a column of zeroes
        if 'LOW' in df.columns:
            selected_column = df['LOW']
        else:
            selected_column = pd.Series([0] * len(df), name='LOW')
    ...
    combined_data.to_csv('combinedImpact_Low.csv', index=False)

    print("Combined data saved to 'combinedImpact_Low.csv'")
