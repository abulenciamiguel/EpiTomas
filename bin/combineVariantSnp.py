...
def combine_selected_columns(csv_files):
    ...
    for csv_file in csv_files:
        ...
        # Extract the "snp" column if it exists, otherwise create a column of zeroes
        if 'snp' in df.columns:
            selected_column = df['snp']
        else:
            selected_column = pd.Series([0] * len(df), name='snp')
    ...
    combined_data.to_csv('combinedType_Snp.csv', index=False)

    print("Combined data saved to 'combinedType_Snp.csv'")
