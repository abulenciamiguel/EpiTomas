...
def combine_selected_columns(csv_files):
    ...
    for csv_file in csv_files:
        ...
        # Extract the "ins" column if it exists, otherwise create a column of zeroes
        if 'ins' in df.columns:
            selected_column = df['ins']
        else:
            selected_column = pd.Series([0] * len(df), name='ins')
    ...
    combined_data.to_csv('combinedType_Insertion.csv', index=False)

    print("Combined data saved to 'combinedType_Insertion.csv'")
